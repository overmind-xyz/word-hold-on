...
module overmind::word_hold_on {
    ...
    const ANSWER_HASH: vector<u8> = x"60c4004508ddcd8d1b0ea1c56ed1e5679d756d72e40f1a00820dbe5d9f69ff63"; // OVERMIND
    const FIRST_LETTER_HASH: vector<u8> = x"3eecb4a5c11c8bab18ddad1d268c827aaabb17c83f51869832a5af15efdedfcb"; // O
    const SECOND_LETTER_HASH: vector<u8> = x"e63a84c18447bfca5c67b20a58fc6a4fefa762e4fa0e6b3b2e46f64daba345e5"; // V
    const THIRD_LETTER_HASH: vector<u8> = x"d034b2b544e4ffb619a9c156ae578fe21f38eb0997f097ca9569807ca157f4f6"; // E
    const FOURTH_LETTER_HASH: vector<u8> = x"6920014bef534e7eea89545a50d6aef0921f1972efcddce9f22f04a45b47d472"; // R
    const FIFTH_LETTER_HASH: vector<u8> = x"c837f30e97185c362830b324e58a3e6782095ee8457109b27f03819ff516e121"; // M
    const SIXTH_LETTER_HASH: vector<u8> = x"c837f30e97185c362830b324e58a3e6782095ee8457109b27f03819ff516e121"; // I
    const SEVENTH_LETTER_HASH: vector<u8> = x"345baaa13bbe3a40695db7697fbe3f64206323b77cf3635902106f9f29667361"; // N
    const EIGHTH_LETTER_HASH: vector<u8> = x"037f4095baddc6f37fde4740c304b1691512d2fc9cf7ede8a93b8c9ec3d1fe07"; // D
    ...
    */
    public entry fun guess_word(player: &signer, word: String) acquires State, Player {
        ...
        let word_hash = hash::sha3_256(*string::bytes(&word));
        if (word_hash == ANSWER_HASH) {
        ...
        if (simple_map::length(&player_resource.attempts) == ATTEMPTS_LIMIT) {
    ...
    */
    inline fun submit_correct_answer(player_resource: &mut Player, player_address: address) {
        ...
        event::emit_event(
            &mut player_resource.submit_correct_answer_events,
            SubmitCorrectAnswerEvent {
                attempt: simple_map::length(&player_resource.attempts),
    ...
    */
    inline fun add_guess_to_attempts(player_resource: &mut Player, player_address: &address, word: String) {
        ...
        while (counter < ANSWER_LENGTH) {
            let letter = string::sub_string(&word, counter, counter + 1);
            let letter_bytes = *string::bytes(&letter);
            let letter_hex = hash::sha3_256(letter_bytes);
            let is_letter_matching = letter_hex == get_letter_hash(counter);

            vector::append(&mut letter_bytes, bcs::to_bytes(&counter));
            vector::append(&mut letter_bytes, bcs::to_bytes(player_address));
            vector::append(&mut letter_bytes, bcs::to_bytes(&timestamp_seconds));
            simple_map::add(&mut result, hash::sha3_256(letter_bytes), is_letter_matching);

            counter = counter + 1;
        };

        // TODO: Get current attempt (length of `attempts` SimpleMap of `player_resource`)
        let attempt = (simple_map::length(&player_resource.attempts) as u8);
        ...
        simple_map::add(&mut player_resource.attempts, attempt, result);

        // TODO: Emit `GuessWordAttemptEvent` event
        event::emit_event(
            &mut player_resource.guess_word_attempt_events,
            GuessWordAttemptEvent {
                attempt,
                word,
                letter_correctness: result,
                event_creation_timestamp_seconds: timestamp_seconds
            }
        )
    }
    ...
    */
    ...
    inline fun check_if_player_has_not_reached_attempts_limit_yet(player_resource: &Player) {
        ...
        assert!(simple_map::length(&player_resource.attempts) < ATTEMPTS_LIMIT, EAttemptsLimitReached);
    }
